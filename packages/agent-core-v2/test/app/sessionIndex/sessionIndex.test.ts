...
describe('FileSessionIndex (read model)', () => {
  ...
  it('get prefers a queued mirror entry over the not-yet-flushed store row', async () => {
    await seedSession('a', { title: 'stored', createdAt: 1, updatedAt: 2 });
    const store = build();
    await store.prepare();

    // A rename + archive flip sits in the mirror queue; the store row still
    // carries the pre-flush values and must lose to the queued summary.
    mirror.record(summary('a', { title: 'renamed', archived: true, updatedAt: 3 }));
    const found = await store.get('a');
    expect(found).toMatchObject({ title: 'renamed', archived: true });

    await mirror.drain();
    const settled = await store.get('a');
    expect(settled).toMatchObject({ title: 'renamed', archived: true });
  });
  ...
  it('cursor-less pages fold a queued archive flip into a removal', async () => {
    await seedSession('a', { createdAt: 1, updatedAt: 2 });
    await seedSession('b', { createdAt: 2, updatedAt: 3 });
    const store = build();
    await store.prepare();

    // Queued but not yet flushed: the stored page still carries
    // `archived: false`, so the pending archive flip must REMOVE the row,
    // not just fail to re-add it.
    mirror.record(summary('b', { archived: true, updatedAt: 4 }));
    const page = await store.listRecent({ workspaceIds: [workspaceId], limit: 20 });
    expect(page.items.map((s) => s.id)).toEqual(['a']);

    const all = await store.listRecent({ workspaceIds: [workspaceId], includeArchived: true });
    expect(all.items.map((s) => s.id)).toEqual(['b', 'a']);

    await mirror.drain();
    const settled = await store.listRecent({ workspaceIds: [workspaceId], limit: 20 });
