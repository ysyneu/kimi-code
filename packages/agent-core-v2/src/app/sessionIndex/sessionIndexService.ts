...
export class FileSessionIndex extends Disposable implements ISessionIndex {
  ...
  private async getFromReadModel(
    ...
    // The mirror queue is fresher than the store: a queued mutation (rename,
    // archive flip) must win over the not-yet-flushed stored row, or reads
    // lose read-your-writes for the whole flush window.
  ...
  private mergePending(
    ...
    const pendingById = new Map(relevant.map((summary) => [summary.id, summary]));
    const merged = new Map<string, SessionSummary>();
    // The queue is newer than the store page, so it wins by id — a queued
    // update replaces the stored row, and a queued archive flip must REMOVE
    // the stored row from the listing, not just fail to re-add it (the store
    // still carries the pre-flush `archived: false` until the flush lands).
    for (const summary of page.items) {
      if (!pendingById.has(summary.id)) merged.set(summary.id, summary);
    }
    for (const summary of relevant) {
      if (query.includeArchived !== true && summary.archived) continue;
      if (
        position !== undefined &&
        !(position.before
