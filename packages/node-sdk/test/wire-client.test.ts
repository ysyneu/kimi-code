...
describe('createKimiHarnessWire', () => {
  ...
  it('resumeSession always refreshes an already-resumed session, even on a cache hit', async () => {
    ...
    try {
      const session = await harness.createSession({ workDir: cwd });
      // The stub provider's turn retries instead of dying instantly; abort
      // it to settle the session quickly, same as the resume-replay test
      // above — the user message is persisted on submit, before the turn
      // runs, which is all this test needs.
      await session.prompt('first message');
      await harness.wireRpc()?.cancel({ sessionId: session.id });
      await waitForAsync(async () => !(await http.getSession(session.id)).busy);

      const resumed = await harness.resumeSession({ id: session.id });
      expect(resumed).toBe(session);
      expect(
        replayUserTexts(resumed.getResumeState()?.agents['main'] ?? { replay: [] }),
      ...
      await rpc?.prompt({
        ...
        input: [{ type: 'text', text: 'second message via bare rpc' }],
      });
