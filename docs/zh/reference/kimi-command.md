...
底部输入框用于在当前工作目录派发新会话：输入任意文本即聚焦输入框，`Enter` 会以该文本作为首条提示词创建会话。输入框支持两个斜杠命令前缀 —— `/model <别名>` 和 `/agent <profile>` 会为新会话的首条提示词指定模型或 Agent 覆盖（如 `/agent reviewer 审查这个分支的改动`）；`/help` 显示帮助。在此处精确输入 `exit` 或 `/exit` 并回车，会改为关闭 Agents 视图，效果与 `Esc` 相同。在某个会话行上按 `Space` 会让同一个输入框切换到回复模式：`Enter` 会把输入的文本原样发给该会话 —— 不创建新会话，也不解析斜杠，回复开头的 `/model`、`/agent` 会作为普通文本原样发出，不会被当成覆盖指令 —— `Esc` 则返回到派发新会话。
