...
打开 Agents 视图 —— 一个基于本地 Kimi 服务的全屏终端仪表盘。它把视图自己拥有的会话 —— 从视图派发或接入的每一个会话 —— 按实时状态分组列出（Pinned / Needs input / Working / Completed，按此顺序），会话有进展或等待输入时实时更新，底部输入框可以直接派发新会话，让你不用离开终端就能同时运行和看护多个会话。

每一行会展示会话的状态、名称、最新一次助手回复的摘要，以及距上次更新过去了多久。状态符号在会话忙碌时是动画效果，有新输出但你还没看过时显示 `✻`，看过之后显示 `∙`。
...
| `→` | 打开会话，效果与 `Enter` 相同；在分组标题上展开已折叠的分组 |
| `←` | 折叠已展开的分组；在已接入的聊天中（输入框为空时）返回 Agents 视图 |
| `Space` | 在底部输入框中直接回复所选会话，不打开其聊天界面 |
| `Shift-↑` / `Shift-↓` | 在 Pinned 分组内重新排序所选会话 |
| `Alt-1` – `Alt-9` | 打开第 N 个可见会话，效果与 `Enter` 相同 |
| `@` | 在底部输入框中按路径提及文件 |
| `Ctrl-J` | 在底部输入框中插入换行 |
| `Shift-Enter` | 在底部输入框中派发新会话并立即接入 —— 仅限纯文本，回复模式下无效，带 `/model` / `/agent` / skill / 插件前缀时也无效 |
| `Ctrl-X` | 归档所选会话（需按两次确认）；在分组标题上归档整组。会话若有正在运行的轮次会先被取消 |
| `Ctrl-R` | 重命名会话 |
| `Ctrl-T` | 置顶 / 取消置顶会话；置顶的会话进入 Pinned 分组 |
| `Ctrl-S` | 在按状态分组（Pinned / Needs input / Working / Completed）和按工作目录分组之间切换 |
| `?` | 显示快捷键列表 |
| `Esc` | 退出 |
| `Ctrl-C` | 双击退出：第一次按下会进入待确认状态，几秒内再按一次才真正退出；期间按其他键会清除待确认状态 |

底部输入框用于在当前工作目录派发新会话：输入任意文本即聚焦输入框，`Enter` 会以该文本作为首条提示词创建会话（`Shift-Enter` 还会立即接入新会话 —— 见上表）。输入框支持斜杠命令前缀：`/model <别名>` 和 `/agent <profile>` 会为新会话的首条提示词指定模型或 Agent 覆盖（如 `/agent reviewer 审查这个分支的改动`），skill 或插件命令（如 `/skill:reviewcode`）则会改为指定该激活项，作为会话的首个动作执行，而不是提示词。CLI 在别处支持的其他斜杠命令 —— 包括 `/help` —— 在这里不可运行；派发这类命令会提示你先接入某个会话。在此处精确输入 `exit` 或 `/exit` 并回车，会改为关闭 Agents 视图，效果与 `Esc` 相同。在某个会话行上按 `Space` 会让同一个输入框切换到回复模式：`Enter` 会把输入的文本原样发给该会话 —— 不创建新会话，也不解析斜杠，回复开头的 `/model`、`/agent` 会作为普通文本原样发出，不会被当成覆盖指令 —— `Esc` 则返回到派发新会话。

#### 服务生命周期

Agents 视图只连接本机上的本地 Kimi 服务：

- **接入已有服务**：当同一 home 目录下已有服务在运行（例如由 `kimi web` 启动），视图会先做一次连通性探测再接入 —— 注册表里已答不应求的残留实例会被当作「没有服务」，转而走内嵌。服务版本必须与当前 CLI 一致，不一致会被拒绝并提示重启。退出只是断开连接 —— 服务和其中的会话继续运行。
...
- 列表只显示本视图在本地 Kimi 服务上创建或接入的会话；直接用 `kimi` 命令启动、或由其他客户端（如 `kimi web`）创建的会话不会出现在列表中。
- 在整个 `kimi agents` 运行期间 —— 无论是在视图内还是在已接入的会话内 —— `!` shell 命令都不可用。
- 在已接入的会话内，`/model` 等配置类斜杠命令目前还不可用。
