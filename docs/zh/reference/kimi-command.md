...
打开 Agents 视图 —— 一个基于本地 Kimi 服务的全屏终端仪表盘。它把视图自己拥有的会话 —— 从视图派发或接入的每一个会话 —— 按实时状态分组列出（Awaiting input / Working / Pinned / Completed），会话有进展或等待输入时实时更新，底部输入框可以直接派发新会话，让你不用离开终端就能同时运行和看护多个会话。

每一行会展示会话的状态、名称、最新一次助手回复的摘要，以及距上次更新过去了多久。
